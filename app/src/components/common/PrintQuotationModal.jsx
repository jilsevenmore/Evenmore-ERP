...
import { X, Printer, CheckCircle2, FileText, Award } from 'lucide-react';

export const PrintQuotationModal = ({ isOpen, onClose, quotation }) => {
    useEffect(() => {
        if (!isOpen) return;
    ...
    const totalAmount = quotation.amount || items.reduce((sum, it) => sum + (it.amount || (it.qty || 1) * (it.rate || 0)), 0);
    ...
    const isConfirmed = quotation.status === 'Confirmed' || quotation.status === 'Converted';
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto print:p-0 print:static print:bg-white print:backdrop-blur-none">
            <div className="bg-white rounded-2xl border border-slate-300 shadow-2xl max-w-4xl w-full my-auto overflow-hidden flex flex-col print:border-none print:shadow-none print:w-full print:max-w-none print:rounded-none">
                ...
                {/* Printable Quotation Document Sheet (A4 Proportion) */}
                <div className="p-8 sm:p-12 text-slate-800 bg-white font-sans text-xs space-y-6 printable-document">
                    ...
                    <div className="flex items-start justify-between border-b-2 border-slate-900 pb-6">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-500 font-semibold tracking-wider uppercase">
                                        Commercial Medical & Industrial Systems Division
                                    </p>
                                </div>
                            </div>
                            <div className="text-[11px] text-slate-500 space-y-0.5 pt-2">
                                ...
                                <p>Commercial Desk: sales@evenmore-erp.com | Phone: +1 (800) 555-0199</p>
                            </div>
                        </div>
                        ...
                        <div className="text-right space-y-1">
                            ...
                            <div className="pt-2 font-mono">
                                <p className="text-lg font-black text-slate-900">{quotation.quoteNumber}</p>
                                <p className="text-xs text-slate-600">
                                    Quote Date: <strong className="text-slate-900">{quotation.date || new Date().toLocaleDateString('en-GB')}</strong>
                                </p>
                                <p className="text-xs text-slate-600">
                                    Validity Window: <strong className="text-slate-900">{quotation.validUntil || '30 Days'}</strong>
                                </p>
                                <p className="text-xs text-blue-700 font-semibold">
                                    Ref: {quotation.estimateRef || 'Direct Quote'}
                                </p>
                            </div>
                            ...
                            <div className="pt-1">
                                {isConfirmed ? (
                                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-600 px-3 py-0.5 rounded uppercase tracking-wider">
                                        <CheckCircle2 size={12} /> CONFIRMED QUOTATION
                                    </span>
                                ) : (
                    ...
                    <div className="grid grid-cols-2 gap-6 bg-slate-50/80 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-slate-600 text-[11px]">Hardware Systems & Equipment Sales Division</p>
                            <p className="text-slate-500 text-[11px]">Origin Warehouse: Central Bay Hub</p>
                        </div>

                        {/* Customer */}
                        <div className="space-y-1 border-l border-slate-200 pl-6">
                            <span className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">
                                QUOTED TO (CLIENT ACCOUNT)
                            </span>
                            <p className="text-sm font-bold text-slate-900">{quotation.customer || 'Client Account'}</p>
                            <p className="text-slate-600 text-[11px]">Commercial Enterprise Account</p>
                            <p className="text-slate-500 text-[11px]">Payment Terms: Net 30 Days upon Delivery</p>
                            <p className="text-slate-500 text-[11px]">Validity: {quotation.validUntil || '30 Days from Issue'}</p>
                        </div>
                    </div>
                                ...
                                {items.length === 0 ? (
                                    <tr>
                                        ...
                                        <td className="py-2.5 px-3">
                                            <p className="font-bold text-slate-900">Commercial Equipment Supply</p>
                                        </td>
                                        <td className="py-2.5 px-3 text-center font-mono">1</td>
                    ...
                    <div className="flex items-start justify-between pt-3 border-t-2 border-slate-800">
                        ...
                        <div className="max-w-xs space-y-1.5 bg-slate-50 p-3 rounded-lg border border-slate-200">
                            <span className="font-bold text-[10px] uppercase text-slate-600 tracking-wider">
                                Bank Wire & Commercial Settlement Desk
                            </span>
                            <p className="text-[10px] text-slate-600">Bank: Chase Manhattan Corporate Banking</p>
                            <p className="text-[10px] text-slate-600 font-mono">Account #: 9948-2019-3321 • Routing: 021000021</p>
                            <p className="text-[10px] text-slate-500 italic">Please quote Quote # on Purchase Order confirmation.</p>
                        </div>
                    ...
                    <div className="pt-8 border-t border-slate-300 grid grid-cols-2 gap-12 text-center text-xs">
                        <div className="space-y-8">
                            <div className="border-b border-slate-400 pb-1 h-12 flex items-end justify-center">
                                <span className="font-mono text-slate-400 text-[10px] italic">Client Purchase Order Confirmation</span>
                            </div>
                            <div>
                                <p className="font-bold text-slate-900">Authorized Client Representative</p>
                                <p className="text-[10px] text-slate-500">Sign & Attach PO to Confirm Order</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                <p className="font-bold text-slate-900">Commercial Operations Manager</p>
                    ...
                    <div className="text-center text-[10px] text-slate-400 pt-4 border-t border-slate-100">
                        Prices valid for {quotation.validUntil || '30 days'}. Standard Evenmore Enterprise warranty and commercial supply agreements apply.
                    </div>
                </div>
                ...
                <div className="bg-slate-100 p-4 border-t border-slate-200 flex justify-between items-center no-print shrink-0">
                    <span className="text-xs text-slate-500">
                        Press <strong>Ctrl+P</strong> or click <strong>Print</strong> to generate an official commercial PDF.
                    </span>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={onClose}
