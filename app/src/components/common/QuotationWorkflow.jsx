...
export function QuotationWorkflow({ quotation, onDownload, onChallan, initialMode = '' }) {
  ...
  const [allowAcceptance, setAllowAcceptance] = useState(quotation.share?.allowAcceptance ?? false);
  ...
  const [message, setMessage] = useState(`Dear ${quotation.customer},\n\nPlease find our quotation ${quotation.quoteNumber} for your review.\n\nYou can view the quotation using the secure link below.\n\nRegards,\nEvenmore ERP`);
  const run = async callback => { setBusy(true); setError(''); try { await callback(); } catch (err) { setError(err.status === 404 ? 'No active share link. Publish a link below.' : 'Sharing service unavailable or authentication required. Check the sharing server configuration.'); } finally { setBusy(false); } };
  useEffect(() => {
    let active = true;
    if (!share?.url) return;
  ...
  const refresh = () => run(async () => { const result = await sharingRequest(quotation.id); setShare(result); setAllowDownload(result.allowDownload); setAllowAcceptance(result.allowAcceptance ?? false); syncQuotationShare(quotation.id, result); });
  const openShare = () => {
    setMode('share');
    if (!sessionStorage.getItem('quotation_admin_token') && !share?.localOnly) publish();
    else refresh();
  };
  const publish = () => run(async () => {
    if (credential) sessionStorage.setItem('quotation_admin_token', credential);
    const result = await sharingRequest(quotation.id, { quotation: publicQuotation(quotation, 'USD'), expiryDays: days, allowDownload, allowAcceptance });
    setShare(result); syncQuotationShare(quotation.id, result); setCredential('');
    showToast(result.localOnly ? 'Quotation preview link generated.' : 'Secure quotation link published.');
  });
  const copy = () => run(async () => {
    ...
    if (!result || Date.parse(result.expiresAt) <= Date.now()) {
      result = await sharingRequest(quotation.id, { quotation: publicQuotation(quotation, 'USD'), expiryDays: days, allowDownload, allowAcceptance });
    }
  ...
  return <section className="quotation-workflow space-y-4">
    ...
    <div className="qw-columns">
      ...
      <div className="space-y-4"><div className="qw-card"><div className="qw-card-title"><ShieldCheck size={17}/><h4>Secure Sharing</h4></div><p>One customer-facing link for the real quotation. Manage expiry and download permissions.</p><Button onClick={openShare}>Share Quotation</Button></div>
    ...
    {mode && <div className="fixed inset-0 z-[65] bg-black/50 flex items-center justify-center p-4"><div role="dialog" aria-modal="true" aria-label={mode === 'share' ? 'Share Quotation' : mode === 'challan' ? 'Convert to Delivery Challan' : 'Send Quotation'} className="qw-dialog bg-white text-slate-800 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-auto space-y-4">
      ...
      {mode === 'challan' ? <><div className="qw-notice"><Truck size={22}/><p>Create a draft delivery challan for <strong>{quotation.customer}</strong> using {quotation.items?.length || 0} quotation items. Review quantities and dispatch details before issuing it.</p></div><p>Source quotation: <strong>{quotation.quoteNumber}</strong></p><Button onClick={() => { setMode(''); onChallan(); }}>Create Delivery Challan</Button></> : mode === 'share' ? <>
        <label className="block">{share?.localOnly ? 'Quotation Preview Link' : 'Secure Share Link'}<input className="w-full border rounded p-2 mt-1" readOnly value={share?.url || ''} placeholder="Publish a secure link"/></label>
        {qr && active && <figure className="qw-qr"><img src={qr} alt="Scan to open this quotation" width="220" height="220"/><figcaption>Scan to view quotation</figcaption></figure>}
        {share?.localOnly && <p className="qw-notice">Preview link works in this browser only. Customer sharing requires a connected service.</p>}
        ...
        <label className="qw-toggle"><input type="checkbox" role="switch" checked={allowDownload} onChange={event => setAllowDownload(event.target.checked)}/>Allow Download</label><label className="qw-toggle"><input type="checkbox" role="switch" disabled={Boolean(share?.localOnly)} checked={allowAcceptance && !share?.localOnly} onChange={event => setAllowAcceptance(event.target.checked)}/>Allow customer acceptance / rejection</label><div className="qw-notice"><ShieldCheck size={19}/><p>{share?.localOnly ? 'This preview uses quotation data saved in your browser.' : 'Anyone with this secure link can view the quotation until it expires. Share it only with your intended recipient.'}</p></div>
        <details><summary>Sharing server connection</summary><p className="text-xs my-2">Enter the administrator token configured on the sharing server. It is kept only for this browser session.</p><input aria-label="Sharing administrator token" type="password" autoComplete="off" value={credential} onChange={event => setCredential(event.target.value)} className="border rounded p-2 w-full"/></details>
        <p className="text-xs">Publishing updates the shared quotation and replaces any previous link.</p>
        ...
        {['Include PDF attachment', 'Include secure link', 'Include QR code'].map(label => <label key={label} className="flex gap-2"><input type="checkbox" defaultChecked/>{label}</label>)}
        <p role="status" className="rounded bg-amber-50 p-3">Email and WhatsApp delivery are not connected. Nothing has been sent and quotation status will stay unchanged. You can copy the secure link to share it yourself.</p>
