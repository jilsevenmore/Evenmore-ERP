import { INITIAL_DEALS, loadDeals } from '../../services/dealService.js';
...
export function seedDemoDealTasks(existing, storage = localStorage, now = new Date()) {
  ...
  const deals = loadDeals(storage).filter((deal) => demoIds.has(deal.id) && !seeded.includes(deal.id));
